...
The phase-2 baseline is commit `39610a58e1cf16dc49ffa08b3f22f3cb34cb6c0e`. The previous phase-1
`bench_asr.json`, `bench_tts.json`, `bench_aec.json`, `summary.json`, and results document are retained
under `results/history/` before phase-2 replacement. `bench_live_latency.json` also keeps the pre-active-
capture run where a measurement-only lead was present, explicitly separated from the active-capture result.
...
not be interpreted as the absolute optimum microphone gain for human speech. Volume changes are bounded to
